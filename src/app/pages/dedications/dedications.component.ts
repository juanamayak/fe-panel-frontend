...
} from "../../components/modals/dedications/create-dedications-modal/create-dedications-modal.component";
import {
...
} from "../../components/modals/dedications/edit-dedications-modal/edit-dedications-modal.component";
...
export class DedicationsComponent implements OnInit{
    ...
    public displayedColumns: string[] = ['title', 'message', 'status', 'action'];
    ...
    openEditDedicationsDialog(dedication): void {
        ...
        const dialogRef = this.dialog.open(EditDedicationsModalComponent, config);
    ...
    deleteDedication(dedicationUuid) {
        this.alertsService.confirmDelete(`¿Estás seguro de eliminar esta dedicatoria?`)
            .then((res) => {
                if (res.isConfirmed) {
                    this.spinner.show();
                    this.dedicationsService.deleteDedications(dedicationUuid).subscribe({
                        next: res => {
                            this.spinner.hide();
